...
if ($kdubah != "") {
  #menampilkan data
  $sql = "SELECT * FROM penyakit WHERE id='$kdubah'";
  $qry = mysqli_query($koneksi, $sql)
    or die("SQL ERROR" . mysqli_error($koneksi));
  $data = mysqli_fetch_array($qry);
  #samakan dengan variabel form
  $id = $data['id'];
  $edit_kdpenyakit = $data['kodePenyakit'];
  $edit_penyakit = $data['namaPenyakit'];
    ...
    <div class="card-body">
      ...
      <form method="post" action="./editpenyakit2.php">
        ...
        <div class="d-grid gap-2">
          ...
          <a href="./penyakit.php" type="button" class="btn btn-danger" name="batal" id="batal">Batal</a>
        </div>
      </form>
    </div>
  </div>
